...
using RealtimeListener.Production.Resilience;

namespace RealtimeListener.Production.Grpc
{
    /// <summary>
    ...
    public class MultiplexedChannelManager : IGrpcConnectionManager, IDisposable
    {
        ...
        /// <param name="apiKey">Optional API key to include in x-api-key header</param>
        ...
        public HubClient.Core.Resilience.ResilientGrpcClient<T> CreateResilientClient<T>(IGrpcResiliencePolicy? resiliencePolicy = null) where T : class
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(MultiplexedChannelManager));
            ...
            return new HubClient.Core.Resilience.ResilientGrpcClient<T>(
                client, 
        ...
        private static GrpcChannel CreateChannel(string endpoint, string? apiKey)
        {
            ...
            // Handle credentials based on whether we're using SSL/TLS
            if (!string.IsNullOrEmpty(apiKey))
            {
                ...
                // Use SSL credentials if the endpoint is HTTPS, otherwise use insecure
            ...
            {
                // Use SSL even without API key if endpoint is HTTPS
